...
export class CompanyTableComponent implements OnInit {
  ...
  constructor(private title: Title,
    private dataService: CompanyService, private toastrService: NbToastrService) {
  }

  ngOnInit(): void {
    this.title.setTitle('پنل مدیریت' + ' | ' + 'لیست ' + 'شرکت ها');

    this.loading = true;
    this.dataService.get().subscribe(
      results => {
        this.source.load(results.data.map(function (val) {
          val.edit = `<a href="/pages/edit/edit-company/${val.id}">link</a>`;
          return val;
        }));
      },
      error => {
